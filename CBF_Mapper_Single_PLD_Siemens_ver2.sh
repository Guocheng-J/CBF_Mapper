...
# v2.3  Added a costume Mask input option, so if you have a T1 GM/WM mask, please use that cuz it produce more robust prediction!
...
# Command line inputs: 
###############################################################
Image_sequence=$1       # Select special sequence ID for CBF Mapping
asl_img=$2     	 # ASL tag-control pairs
pdw_img=$3		 # Proton-density image
T1=$4     		 # Specify the path of T1w for image segmentation. If you don't have that, enter "1" for bypassing.
			 # The software will use PDw image to estimate GM/WM mask. However, this will be less accurate!
			 # If you don't have T1 and use PDw image for estimation, please always do a QC!
...
# Modify the following codes based on how many T-C pairs and
# whether they have same TIS and repeats value
...
###############################################################
# A quick self check on whether the command is valid:  
...
echo "Start analysing CBF for the following patient ID:" ${Patient_name}
...
then
    ...
    echo "FSL: Caution: GM/WM mask estimate from PDw is not idea! Use T1w image is preferred!"
...
then
    ...
    echo "Mode 0 selected: SIemens Prisma 3T pcASL direct estimate"
...
###############################################################
# Step 2.1b: Danny Wang Sequence Siemens Prisma 3T (RCAF Study)
#
...
then
    echo "Mode 1 selected: Danny Wang sequence, Siemens Prisma 3T pcASL"
